...
function OrdersTable({orders,admin}:{orders:ordersType[]|null,admin:null|boolean}):ReactElement {
  ...
  const handleCheckout = useCallback(async(id:string)=>{
    try{
        const {data}:{data:{url:string|undefined,message:string|undefined,status:boolean|undefined}} = await axios.get(import.meta.env.VITE_API_BASE_URL+`/cart/checkoutForExistsOrder/${id}?cancel_url=https://omer-khaled.github.io/failedPage&success_url=https://omer-khaled.github.io/successPage`,{
            headers:{
        ...
        // window.location.href = "https://omer-khaled.github.io/failedPage"
    }
  },[]);
